...
% @rsetienne:
% At the moment, there are 87 parameter combinations for one site and one clock model
% This allows for 40K / 87 = 460 replicates [*]
% Using two site and two clock models would result in approx 100 replicates
% Using four site and four clock models would result in approx 25 replicates
%
% I personally think the broadest data set will be most interesting,
% because it gives a rough idea, but -more importantly- shows the scientific
% community it can easily be done nowadays.
%
% [*] the number of replicates can only be determined by actually running the
% experiment, as there are multiple constraints that I cannot test for 
...
95\% quantile has less than 1250 extant good species. The quantile is
calculated with a recently added function to the PBD package,
inspired on equation 6 of \cite{etienne2014}. This calculation
...
but the GTR model is the site model with the fewest assumptions and most flexibility.
\textbf{[NOTE: Why then not use all four site models?]}
For our clock model, we assume either a strict or relaxed log-normal \textbf{[NOTE: 
Why that one? Why then not use all clock models?]} clock model. The strict clock
model is the correct one, as it is used for simulating that alignment,
but the relaxed log-normal clock model is the model with fewer assumptions and
more flexibility.

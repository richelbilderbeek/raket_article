...
\RAMPAL{I think we should release the new version with this function with this manuscript}
\richel{I would personally prefer 'release early, release often', but as lead maintainer you get to decide}
\RAMPAL{Do you want to put the derivation here?}
\richel{No, as you've added it to the PBD::pbd\_geom and PBD::pbd\_numspec\_quantile) documentation, so I will transfer it. Thanks!}
This calculation assumes $b = b_g = b_i$, we used $b = \max(b_g, b_i)$.
...
rate of good species $b_g \in B$ and incipient species $b_i \in B$.
\RAMPAL{your units are not entirely correct, as these are probability rates.}
\richel{I (perhaps naively) think I am correct in my units. If I am wrong, what are the correct units then? In Etienne and Rosindell 2012, there is never 'probability rate' written. To be explicit, I think theis sentence is valid: the extinction rate of good species, mu\_g has an expected 1.2 extinction events per time unit. What would be the better unit?}
\RAMPAL{Je hebt het over de eenheden van de rates. Dat is subtiel omdat het om een stochastisch model gaat en dus om kansen. Voorbeeld: mu * dt is de kans op een extinction event per species in een tijdsinterval dt. 1/mu geeft de verwachte tijd tot een extinctie event van een soort. mu is dus een probability rate.}
\richel{'mu is dus een probability rate' with what unit? You can redirect me to an article with the proper wording, or finish this sentence: 'the extinction rate of good species, mu\_g ...'. Thanks!}
...
% \RAMPAL{Why not include that in the paper here; I think it is essential}
% \richel{Because we are limited to two figures}
...
are equally likely. 
\richel{New:}
The DNA sequence used for the root ancestor has an equal amount of each nucleotide.
\richel{End of new}

...
They allow to known or hypothesized structure of a phylogenetic 
tree-to-be-inferred to be incorporated through model priors. 
From these priors and an alignment 
of DNA, RNA or protein sequences, they create a posterior distribution
...
% RSE: perhaps move to discussion
...
% Also, a BD model places the most recent common 
...
Because of the paraphylies there is no such thing as a true species tree in the PBD model. 
...
As the analytic solution for the expected number of lineages 
is too complex to be derived (\cite{etienne_and_rosindell_2012}),
we use a numerical approximation instead \textbf{NOTE: maybe Lambert et al., 2014, 
but the (closed access) article is unavailable from home}.
...
For the speciation initiation rate $b$, we use $0.1$, $0.5$ and $1.0$ 
...
events per (incipient species) lineage per time unit. We used $10^9 \approx \infty$
...
are equally likely. 
In our Bayesian inference (see below) 
we use the same site model as the (obviously correct) site model prior.
...
fixed clock rate, I do not see why]}. The tree prior assumed in inference is the BD model, 
...
The nLTT statistic is equal to the area between the normalized
...
maintains information about the distribution.

...
To do so, the mutation rate is set to expect on average 1 (possibly silent) mutation per nucleotide
between crown age and present, which equates to $\frac{1}{15}$ mutations
per million year.
The DNA sequence length is chosen to provide a
resolution of a 1K years, that is, to have one expected nucleotide change 
per 1k years per lineage on average. As 1 nucleotide is expected 
to have on average 1 (possibly silent) mutation per 15 million years, 15K
nucleotides provides for 1 mutation per alignment per 1K years (which is
...
alignment is created. The MCMC chain is run to generate 1111 states,
...
To provide for copying such a table (thus, allowing it to be present in
memory twice), the size of the table has an upper limit of half its maximal size.
Each row will contain the parameter set and the generated nLTT 
statistics (see \ref{table:specs} for the exact data specification).
From these constraints, this allows for 20K rows. 

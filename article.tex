...
BEAST (\cite{beast}) and its extension \RAMPAL{it is not really a successor, more a branch} BEAST2 (\cite{beast2}), 
...
they create \rampal{a sample of} the posterior distribution
of \rampal{phylogenies and parameter estimates} (of the models used as a prior), 
...
The choice of \RAMPAL{delete: a wrong} site model (\cite{posada_and_buckley_2004}), 
...
\rampal{Furthermore, while we evidently know the clock and site models used in the simulation, using a different clock and/or site model prior in inference may compensate or increase this difference between inferred and simulated tree. To study this, we also explore the effect of 
...
This quantile is
calculated with \rampal{the pbd\_numspec\_quantile function we added to the} \verb;PBD; package (\cite{pbd}).
\RAMPAL{I think we should release the new version with this function with this manuscript. I deleted the reference to equation 6, because it is incorrect. Do you want to put the derivation here?}. This calculation
...
For the speciation initiation rates of good and incipient species, we use $b_g = 0.35$ and $b_i = 0.55$, 
respectively.\RAMPAL{Jouw units zijn niet helemaal correct, want het zijn probability rates.}
...
The extinction rates of good and incipient species used 
are $\mu_g = \mu_i = 0.0$, $0.1$ and $0.2$.\RAMPAL{I made some changes here as well}
...
all with a crown age of 15 million years \rampal{as used in \cite{etienne2014}.}
Each protracted birth-death tree uses a different random number
\rampal{generator} seed, which makes all runs independent, resulting in a balanced 
data set. \richel{Rampal assumed runs with close seeds were related. 
I assume I have convinced him otherwise]} \rampal{I am not saying that close seeds mean correlation; we simply don't know whether two lists of pseudo-random numbers generated from the two different seeds are not correlated, but perhaps the easiest way to resolve this is to check this for a fairly long list of numbers}
...
See the supplementary information for a visualization of these sampling methods.\RAMPAL{Why not include that in the paper here; I think it is essential}
...
that there will be more noise and hence our inference error will increase.\RAMPAL{I am not sure: perhaps we compensate for the error made by adding more flexibility in another prior}
...
which we will specify as one of the two clock models assumed in the Bayesian inference \rampal{(see below)}.
...
clock model. 
Also here, the strict clock model is the correct one, as it is used for simulating \rampal{the} alignment,
...
Of the remaining
1000 MCMC states, the effective sample size (ESS) of the posterior \richel{I chose 
...
\rampal{using} the nLTT statistic (\cite{janzen2015}), \rampal{from} the nLTT package (\cite{nltt}). 
...
of CPU time and 20 days of wall clock time per replicate. 
Due to this, we choose to perform ten replicates, so that the complete
experiment will take an acceptable time of roughly seven months. \RAMPAL{So this includes queue time? Perhaps mention this.}

For both data sets, we display the nLTT statistics distribution per
biological parameter combination as a violin plot.
We only show combinations for which $\lambda_g = \lambda_i$ and $\mu_g = \mu_i$ \RAMPAL{So where are the other results? All results should at least be in the supplementary material},
to simplify the interpretation of \rampal{the} results. 
...
and a strict clock model. We display the nLTT statistic distributions for \RAMPAL{replace 'both' by 'other'?}both
site and clock models in the supplementary information. {Finally}, we show the effect of sampling,
by \rampal{separating} the nLTT statistics distribution per sampling method used.
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    balanced data set, 
    for the subset of combinations in which $\lambda_g = \lambda_i$, $\mu_g = \mu_i$, 
...
\begin{table}
  ...
  \begin{tabular}{l p{0.65\textwidth}}
    ...
    Protracted speciation & The process in which speciation takes two events \RAMPAL{delete: to complete}: a speciation-initiation event and a speciation-completion event  \\
    Speciation initiation & The start of a speciation event creating an incipient species \\
    Speciation completion & The end of a speciation event, in which an incipient species \rampal{becomes or }is recognized as a good species \\
    \hline
  \end{tabular}
  \caption{
    Glossary
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    using the balanced data set\RAMPAL{what is a balanced data set? Did you define that anywhere?}, 
    for the subset of combinations in which $\lambda_g = \lambda_i$, $\mu_g = \mu_i$, 
...
\begin{table}
  ...
  \caption{
    ...
    The sampling methods are abbreviated as such: \rampal{'R'} denotes random
    ...
    The clock models are abbreviated as 'S' \rampal{for} a strict and 'RLN' \rampal{for} a relaxed log-normal model.
    The site models are abbreviated as 'JC69' for Jukes-Cantor \RAMPAL{cite the JC 1969 paper?} and 'GTR' for the generalized 
...
\begin{table}
  ...
  \end{tabular}
  \caption{
    Overview of the 11 \rampal{parameters estimated by BEAST2}
  }

...
\textbf{[NOTE: I will propose different parameter combinations, due to computational and aesthetic problems]}
...
\textbf{[NOTE: accoring to Rampal this has been solved analytically (for the Protracted Birth and Death model). Where?]}.
...
that we use are $0.0$, $0.1$, $0.2$ and $0.4$ 
...
data set. \textbf{[NOTE: maybe discuss with Rampal to remove confusion. Rampal requests a histogram of branch lengths to follow a geometric distribution]}
...
it is the wiser choice over 'prior' and parameter estimates. Must discuss]} must at least be 200

...
The per species speciation completion rates $\lambda$ we use are $0.1$, $0.3$, $1.0$ and $10^9$. 
This means that there is a $0.15\thinspace dt$ ($0.35\thinspace dt$, $1.0\thinspace dt$, $10^9\thinspace dt$) 
...
\richel{
  Note there is no twinning, as in the razzo package.
  Already sent an email to discuss this with RSE.
}
...
\richel{
  There are no rules described in the setup of the NS
  (for example, what should the relation between MCMC chain length
  and NS sub-chain length be?),
  but one can experiment with a sub-chain length and number of particles.
  I will need to do some pilot studies to verify if this is
  the correct way to do and describe this.  
}
\richel{End new}
...
as a stacked bar. 
\richel{
  I am unsure how to depict that stacked bar and its uncertainty.
  Sure, the stacked bars (without uncertainty) will work,
  but information about uncertainty is lost. 
  See figure 'Showing a stacked bar with uncertainty'
  for three equivalent ways to display a stacked bar with uncertainty.
}

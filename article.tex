...
which we will specify as one of the two clock models assumed in the Bayesian inference.
...
We only show combinations for which $\lambda_g = \lambda_i$ and $\mu_g = \mu_i$,
to simplify the interpretation of these results. 
...
and a strict clock model. We display the nLTT statistic distributions for both
site and clock models in the supplementary information. We show the effect of sampling,
by seperating the nLTT statistics distribution per sampling method used.
